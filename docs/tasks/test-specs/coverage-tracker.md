...
| 100 | Transitive edge model + lockfile graph reader (ADR 009 Decision 1) | 100-transitive-edge-model-lockfile-graph-reader-test-spec.md | 18/18 | 🟡 code merged — NodeId (Registry/Git, no third variant) + DependencyGraph (from_edges cycle-safe, nodes, edges_from) defined; package_lock_json_to_graph (v1/v2/v3 edges), cargo_lock_to_graph (registry+git NodeId), requirements_txt_to_graph and go_sum_to_graph (explicit-empty edge contract documented); unresolvable edges emit diagnostic not panic; zero network I/O. L4: cargo test 1020/1020 passed; clippy --all-targets --all-features -D warnings clean; fmt --check clean. |
