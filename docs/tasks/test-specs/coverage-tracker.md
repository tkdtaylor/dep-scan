...
| 101 | Manifest edge reader for fetched git sub-trees (ADR 009 Decision 1, manifest-fallback) | 101-manifest-edge-reader-git-subtrees-test-spec.md | 13/12 (T-101-01..12 + T-101-07b) | 🟡 code merged — manifest_edges_from_tree reads package.json/Cargo.toml from FetchedTree bytes; zero network I/O structural guarantee; devDependencies excluded; absent/malformed → Err(ManifestError) never silent empty; ecosystem hint selects exactly one manifest; git dep with rev → NodeId::Git; without rev → UnresolvedRange; workspace root no [dependencies] → empty Ok. Verified by: L4: cargo test 1033/1033 passed; clippy -D warnings clean; fmt clean. |
