...
| 107 | [transitive] config block + CLI enable/disable flag (ADR 009 piece 7 — Decisions 2a/3b config shape) | 107-transitive-config-block-cli-flag-test-spec.md | 19/19 (T-107-01..19) + t107_depth_limit_action_to_on_depth_limit_conversion | 🟡 code merged — src/config.rs: DepthLimitAction enum (Warn default/Block; #[derive(Default)] + #[default] on Warn; serde rename_all=lowercase; unknown values rejected by serde). TransitiveConfig struct with serde defaults: enabled=false, max_depth=5, on_depth_limit=Warn, fetch_concurrency=4, max_total_nodes=5000. Validated in Config::from_toml_str: fetch_concurrency=0 → error (deadlock prevention, REQ-107-03); max_total_nodes=0 → error (fail-closed budget). max_depth=0 accepted (depth-0: root only, children cut by DepthLimitReached, documented). Config::transitive field added + Default impl updated. From<DepthLimitAction> for OnDepthLimit: 1:1 mapping keeps engine decoupled from serde. src/cli.rs: --transitive / --no-transitive bool flags (overrides_with mutual override). resolve_transitive(transitive, no_transitive) → Option<bool>: Some(true)/Some(false)/None. src/main.rs: Command::Check destructures transitive+no_transitive; resolve_transitive applied; run_check gains transitive_override: Option<bool> parameter; config.transitive.enabled overridden when Some. Verified by: L2 unit (cargo test 1104/1104 passed, all 20 T-107 tests green) + L3 fitness (clippy -D warnings clean, fmt check clean). No CI configured (L4 N/A). |
