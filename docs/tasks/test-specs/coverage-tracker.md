...
| 096 | VCS fetch client — sandboxed, read-only fetch (ADR 008 piece 2 — fetch) | 096-vcs-fetch-sandbox-test-spec.md | 19/21 (T-096-20 N/A — pure-Rust only, no shell-out path to compare; T-096-21 = tooling gate) | 🟡 code merged — pure-Rust gix fetch-to-objects + materialise-ourselves sandbox; all sandbox sentinels are real behavioural tests over a local git daemon (T-096-06 hooks, -07 submodules, -08 symlinks, -09 traversal, -10 absolute); fail-closed wiring (T-096-15). Verified by L3 fitness (clippy -D warnings + fmt + cargo audit 0 advisories) + L6 operator (ran binary vs git-dep lockfile, denied-host fail-closed warn observed). No CI configured (L4 N/A). Awaiting spec-verifier before ✅. |
