...
| 101 | Manifest edge reader for fetched git sub-trees (ADR 009 Decision 1, manifest-fallback) | 101-manifest-edge-reader-git-subtrees-test-spec.md | 13/12 (T-101-01..12 + T-101-07b) | 🟡 code merged — aligned to ADR 011: Resolved emitted ONLY for git+rev (NodeId::Git); ALL other edges Unresolved including Cargo table-with-version (range = verbatim version string). T-101-04 asserts both serde("1.0") and tokio("1") Unresolved; T-101-12 asserts range == "https://github.com/example/mylib" verbatim; T-101-10 asserts serde Unresolved. manifest_edges_from_tree reads package.json/Cargo.toml from FetchedTree bytes; zero network I/O; devDependencies excluded; absent/malformed → Err(ManifestError); ecosystem hint selects one manifest; workspace root no deps → empty Ok. Verified by: L4: cargo test 1033/1033 passed; clippy -D warnings clean; fmt clean. |
