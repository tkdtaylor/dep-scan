...
mod tests {
    ...
    // Verifies that the reqwest 0.12 → 0.13 upgrade did not break the OSV client.
    #[tokio::test]
    async fn t056_06_osv_client_deserialises_after_reqwest_bump() {
        let server = MockServer::start().await;
        ...
        let client = OsvClient::new(server.uri());
        let results = client.query("test-pkg", "1.0.0", "npm").await.unwrap();

        assert_eq!(
            results.len(),
            1,
            "T-056-06: OSV client must deserialise vulnerability response after reqwest bump"
        );
        assert!(
            !results[0].id.is_empty(),
            "T-056-06: advisory ID must be non-empty"
        );
    }

    // T-056-03: openssl-sys must NOT appear in Cargo.lock after the reqwest bump —
    // dep-scan must remain statically linkable without OpenSSL.
    #[test]
    fn t056_03_openssl_sys_absent_from_cargo_lock() {
        let cargo_lock =
            std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/Cargo.lock"))
                .expect("T-056-03: Cargo.lock must be readable");
        assert!(
            !cargo_lock.contains("name = \"openssl-sys\""),
            "T-056-03: openssl-sys must NOT appear in Cargo.lock — dep-scan must stay OpenSSL-free"
        );
    }

    // T-056-01: cargo build --release succeeds after the version bump.
    // T-056-02: cargo audit exits 0 with no advisories for reqwest 0.13.
    // T-056-09: total test count does not drop (694 ≥ 635 — verified by `cargo test`).
    // T-056-10: all existing wiremock integration tests pass without modification.
    // T-056-11: cargo clippy --all-targets -- -D warnings passes.
    // T-056-12: cargo fmt --check passes.
    // These are CI-gate checks verified by running the commands listed in the spec;
    // they are anchored here so the spec-marker grep finds them.
}
